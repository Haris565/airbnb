...
function Header({placeholder}) {
    ...
    return (
        <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md py-5 px-5 md:px-10 ">
            ...
            <div className='flex items-center border-2 rounded-full py-2 md:border md:shadow-sm '>
                <input type="text" value={searchInput} onChange={(e)=>setsearchInput(e.target.value)} 
                placeholder={placeholder || "Start your Search"} className='pl-5 bg-transparent outline-none flex-grow text-sm text-gray-600 placeholder-gray-300 ' />
            ...
            <div className='flex flex-col col-span-3 mx-auto'>
                ...
                <div className='flex items-center border-b mb-5' >
                    <h2 className="text-2xl flex-1 font-semibold text-red-400">Number of Guest</h2>
                    <UserIcon className='h-5 text-red-400' />
                    <input type="number" value={numberOfGuest} onChange={(e)=>setnumberOfGuest(e.target.value)} min={1} className='w-12 pl-2 text-lg outline-none text-red-400' />
                </div>
